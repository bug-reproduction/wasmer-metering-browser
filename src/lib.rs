...
fn create_wasm_module(wasm_bytes: &mut [u8]) -> WasmModule {
    ...
    fn seed(_env: FunctionEnvMut<MyEnv>) -> f64 {
        return 1.0; // TODO
    }
    let seed = Function::new_typed_with_env(&mut store, &env, seed);
    ...
    let import_object = imports! {
        "env" => {
            "print_u32" => print_u32,
            "seed" => seed  // required by assemblyscript if using Math.random
        },
    };
...
impl DualExecution {
    ...
    pub fn add_module(&mut self, wasm_bytes: &mut [u8]) {
        if let Some(_existing) = &self.module1 {
            if let Some(_existing) = &self.module2 {
                panic!("already both module added");
            } else {
